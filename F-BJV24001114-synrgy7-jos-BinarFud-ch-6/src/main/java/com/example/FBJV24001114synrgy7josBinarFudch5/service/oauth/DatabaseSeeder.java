...
import com.example.FBJV24001114synrgy7josBinarFudch5.model.entity.oauth.RolePath;
import com.example.FBJV24001114synrgy7josBinarFudch5.model.entity.oauth.AuthUser;
import com.example.FBJV24001114synrgy7josBinarFudch5.repository.oauth.ClientRepository;
import com.example.FBJV24001114synrgy7josBinarFudch5.repository.oauth.RolePathRepository;
import com.example.FBJV24001114synrgy7josBinarFudch5.repository.oauth.RoleRepository;
...
public class DatabaseSeeder implements ApplicationRunner {
    ...
    public void insertUser(String password) {
        for (String userNames: users) {
            ...
            Optional<AuthUser> oldAuthUser = userRepository.findOneByUsername(username);
            if (oldAuthUser.isEmpty()) {
                oldAuthUser = Optional.of(new AuthUser());
                oldAuthUser.get().setUsername(username);
                oldAuthUser.get().setPassword(password);
                List<Role> r = roleRepository.findByNameIn(roleNames);
                oldAuthUser.get().setRoles(r);
            }

            userRepository.save(oldAuthUser.get());
        }
    }
}
